    ...
    <div class="processes-view">
                ...
                <div class="header-row">
                    <div class="header-title">
                        <h1 class="page-title">
                            Processes
                            <span
                                v-if="processStore.processCount > 0"
                                class="stats-inline"
                            >
                                · {{ stats.running }} running,
                                {{ stats.stopped }} stopped<template
                                    v-if="stats.failed > 0"
                                    >, {{ stats.failed }} failed</template
                                >
                            </span>
                        </h1>
                    </div>
                    <div class="header-actions">
                        ...
                        <button
                            ...
                            class="action-btn"
                            :class="{
                                active: settingsStore.viewMode === 'card',
                            }"
                            :title="t('views.card')"
                        ...
                        <button
                            ...
                            class="action-btn"
                            :class="{
                                active: settingsStore.viewMode === 'table',
                            }"
                            :title="t('views.table')"
        ...
        <div class="page-body">
            <div class="container-xl">
                ...
                <div
                    v-else-if="
                        !processStore.loading && filteredProcesses.length === 0
                    "
                    class="empty"
                >
                    ...
                    <p class="empty-subtitle text-muted">
                        {{
                            processStore.processCount === 0
                                ? "Create your first process from a template"
...
const stats = computed(() => {
    const running = processStore.processes.filter((p) =>
        isRunning(p.state),
    ).length;
    const stopped = processStore.processes.filter(
        (p) => isStopped(p.state) || isNotStarted(p.state),
    ).length;
    const failed = processStore.processes.filter((p) =>
        isFailed(p.state),
    ).length;
