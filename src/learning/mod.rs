...
impl LearningEngine {
    ...
    async fn update_patterns(
        ...
        debug!("Updated pattern for {}: confidence = {}", process_id, pattern.confidence);
    }
    ...
    async fn process_event(&self, event: ProcessEvent) -> Result<()> {
        ...
        match event.event_type {
            ...
            EventType::ProcessError => {
                self.learn_process_error(&event.process_id, event.context).await?;
            }
            _ => {}
        }
    ...
    async fn learn_process_error(&self, process_id: &str, context: Option<serde_json::Value>) -> Result<()> {
        // エラーパターンを記録
        let client = self.db.client().await;
                ...
                for pattern in patterns {
                    for next_process in &pattern.next_processes {
                        suggestions.push(Suggestion {
                            ...
                            reason: format!("過去のパターンから学習（信頼度: {:.0}%）", pattern.confidence * 100.0),
                        });
                    }
                }
            }
