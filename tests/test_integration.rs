...
async fn test_process_basic_lifecycle() {
    ...
    // Start the process
    let pid = manager.start_process("basic-test".to_string()).await
    ...
    // Get output
    let output = manager.get_process_output(
        "basic-test".to_string(),
        ichimi_server::process::types::LogStream::Stdout,
        Some(10)
    ).await.expect("Failed to get output");
    
    assert!(!output.is_empty());
    assert!(output[0].contains("Hello, Ichimi!"));
    
    // Clean up
    manager.remove_process("basic-test".to_string()).await
...
async fn test_process_with_environment() {
    ...
    let pid = manager.start_process("env-test".to_string()).await
        .expect("Failed to start process");
    assert!(pid > 0);
    ...
    // Clean up
    manager.remove_process("env-test".to_string()).await
        .expect("Failed to remove process");
}
...
async fn test_long_running_process_management() {
    ...
    manager.create_process(
        ...
        "sh".to_string(),
        vec!["-c".to_string(), "while true; do echo 'Still running...'; sleep 1; done".to_string()],
        HashMap::new(),
        None,
    ).await.expect("Failed to create process");
    
    // Start the process
    let pid = manager.start_process("long-runner".to_string()).await
        .expect("Failed to start process");
    assert!(pid > 0);
    ...
    // Verify it's running
    let status = manager.get_process_status("long-runner".to_string()).await
        .expect("Failed to get status");
    assert!(matches!(status.info.state, ichimi_server::process::types::ProcessState::Running { .. }));
    ...
    manager.stop_process("long-runner".to_string(), Some(2000)).await
    ...
    // Clean up
    manager.remove_process("long-runner".to_string()).await
        .expect("Failed to remove process");
}
...
async fn test_multiple_concurrent_processes() {
    ...
    for i in 1..=num_processes {
        manager.create_process(
            format!("concurrent-{}", i),
            "sh".to_string(),
            vec!["-c".to_string(), format!("echo 'Process {} started'; sleep 0.5; echo 'Process {} done'", i, i)],
            HashMap::new(),
            None,
        ).await.expect(&format!("Failed to create process {}", i));
    }
    ...
    for i in 1..=num_processes {
        ...
        let handle = tokio::spawn(async move {
            manager_clone.start_process(format!("concurrent-{}", i)).await
        });
        handles.push(handle);
    }
    ...
    for i in 1..=num_processes {
        let output = manager.get_process_output(
            ...
            ichimi_server::process::types::LogStream::Stdout,
            Some(10)
        ).await.expect(&format!("Failed to get output for process {}", i));
        
        assert!(!output.is_empty());
        assert!(output.iter().any(|line| line.contains(&format!("Process {}", i))));
    }
    
    // Clean up all processes
    for i in 1..=num_processes {
        manager.remove_process(format!("concurrent-{}", i)).await
...
async fn test_process_error_handling() {
    ...
    // Start the process
    let pid = manager.start_process("failing-process".to_string()).await
        .expect("Failed to start process");
    assert!(pid > 0);
    ...
    let status = manager.get_process_status("failing-process".to_string()).await
        .expect("Failed to get status");
    ...
    // Clean up
    manager.remove_process("failing-process".to_string()).await
        .expect("Failed to remove process");
}
...
async fn test_process_filtering() {
    ...
    for (id, cmd, args) in &test_processes {
        manager.create_process(
            ...
            HashMap::new(),
            None,
        ).await.expect(&format!("Failed to create {}", id));
    }
    
    // Start some processes
    manager.start_process("filter-running".to_string()).await
        .expect("Failed to start filter-running");
    manager.start_process("filter-echo".to_string()).await
    ...
    // Clean up
    manager.stop_process("filter-running".to_string(), Some(1000)).await.ok();
...
async fn test_process_restart() {
    ...
    // Start it first time
    let pid1 = manager.start_process("restart-test".to_string()).await
    ...
    // Get first output
    let output1 = manager.get_process_output(
        "restart-test".to_string(),
        ichimi_server::process::types::LogStream::Stdout,
        Some(10)
    ).await.expect("Failed to get first output");
    ...
    // Start it again
    let pid2 = manager.start_process("restart-test".to_string()).await
    ...
    tokio::time::sleep(Duration::from_millis(200)).await;
    let output2 = manager.get_process_output(
        "restart-test".to_string(),
        ichimi_server::process::types::LogStream::Stdout,
        Some(10)
    ).await.expect("Failed to get second output");
    
    // Should contain output from both runs
    assert!(output2.iter().any(|line| line.contains("Second run")));
    
    // Clean up
    manager.remove_process("restart-test".to_string()).await
        .expect("Failed to remove process");
}
...
async fn test_process_output_buffering() {
    ...
    // Start the process
    manager.start_process("buffer-test".to_string()).await
        .expect("Failed to start process");
    ...
    let output_10 = manager.get_process_output(
        "buffer-test".to_string(),
        ichimi_server::process::types::LogStream::Stdout,
        Some(10)
    ).await.expect("Failed to get 10 lines");
    assert!(output_10.len() <= 10);
    
    // Test getting all lines (up to buffer limit)
    let output_all = manager.get_process_output(
        "buffer-test".to_string(),
        ichimi_server::process::types::LogStream::Stdout,
        None
    ).await.expect("Failed to get all lines");
    assert!(output_all.len() >= 10);
    
    // Clean up
    manager.remove_process("buffer-test".to_string()).await
        .expect("Failed to remove process");
}
