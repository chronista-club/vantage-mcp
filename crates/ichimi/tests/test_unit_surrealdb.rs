...
async fn test_surrealdb_direct_save_and_load() {
    ...
    assert_eq!(loaded_process.process_id, "test-direct-save");
    assert_eq!(loaded_process.command, "echo");
    assert_eq!(loaded_process.args, vec!["hello", "world"]);
    assert_eq!(loaded_process.auto_start, true);
}
...
async fn test_surrealdb_multiple_processes() {
    ...
    for i in 1..=5 {
        ...
        process.auto_start = i % 2 == 0; // Even numbers are true
    ...
    for i in 1..=5 {
        ...
        assert_eq!(process.auto_start, i % 2 == 0);
    }
}
