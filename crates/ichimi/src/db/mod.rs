...
impl Database {
    ...
    pub async fn export_to_file(&self, path: &std::path::Path) -> Result<()> {
        ...
        // 個別にクエリを実行
        let mut processes_result = client
            .query("SELECT * FROM process")
            .await
            .context("Failed to fetch processes for export")?;
        
        let processes: Vec<ProcessRecord> = processes_result.take(0).unwrap_or_default();
        
        let mut events_result = client
            .query("SELECT * FROM process_event")
            .await
            .context("Failed to fetch events for export")?;
        
        let events: Vec<EventRecord> = events_result.take(0).unwrap_or_default();
        
        let mut templates_result = client
            .query("SELECT * FROM template")
            .await
            .context("Failed to fetch templates for export")?;
        
        let templates: Vec<crate::process::template::ProcessTemplate> = 
            templates_result.take(0).unwrap_or_default();
