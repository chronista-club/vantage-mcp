...
**重要**: mainブランチは保護されており、直接プッシュはできません。すべての変更はプルリクエスト経由で行う必要があります。

### 必須要件
- ✅ プルリクエストレビュー: 最低1人の承認が必要
- ✅ 古いレビューの無効化: 新しいコミット時に再承認が必要
- ✅ 会話の解決: すべてのコメントが解決されるまでマージ不可

### 開発フロー
1. フィーチャーブランチで作業
2. プルリクエストを作成
3. レビュー承認を取得（@makoto.ito）
4. マージ（Squash and merge推奨）

詳細は `.github/BRANCH_PROTECTION.md` を参照してください。
