...
async fn test_process_manager_export_import() {
    ...
    // Export processes
    let exported_path = manager1.export_processes(Some(export_path_str.clone())).await.unwrap();
    ...
    let test_processes: Vec<_> = processes.iter()
    ...
    for i in 1..=3 {
        let process = test_processes.iter()
...
async fn test_process_lifecycle_with_persistence() {
    ...
    manager.export_processes(Some(export_path.to_str().unwrap().to_string())).await.unwrap();
    
    // Start the process
    let pid = manager.start_process("lifecycle-test".to_string()).await.unwrap();
    assert!(pid > 0);
    
    // Get status while running
    let status = manager.get_process_status("lifecycle-test".to_string()).await.unwrap();
    ...
    let _ = manager.stop_process("lifecycle-test".to_string(), Some(1000)).await;
    
    // Remove the process
    manager.remove_process("lifecycle-test".to_string()).await.unwrap();
...
async fn test_export_with_environment_variables() {
    ...
    let processes = manager2.list_processes(None).await;
    let env_process = processes.iter()
        .find(|p| p.id == "env-test")
        .expect("env-test process not found");
    
    assert_eq!(env_process.env.len(), 3);
    assert_eq!(env_process.env.get("TEST_VAR_1"), Some(&"value1".to_string()));
    assert_eq!(env_process.env.get("TEST_VAR_2"), Some(&"value2".to_string()));
    assert_eq!(env_process.env.get("PATH_OVERRIDE"), Some(&"/custom/path".to_string()));
...
async fn test_import_error_handling() {
    ...
    let result = manager.import_processes(invalid_file.to_str().unwrap()).await;
    assert!(result.is_err());
}
...
async fn test_concurrent_export() {
    ...
    for i in 1..=3 {
        ...
        let handle = tokio::spawn(async move {
            manager_clone.export_processes(Some(export_path_str)).await
        });
        handles.push(handle);
    }
